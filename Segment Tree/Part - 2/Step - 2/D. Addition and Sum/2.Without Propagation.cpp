...
struct segment_tree {
	...
	void Modify(int node, int l, int r, int ql, int qr, int val) {
		if (l >= qr or r <= ql) {
			...
			apply_mod_op(values[node], val, r - l);
		} else {
			int mid = (l + r) / 2;
			Modify(node * 2 + 1, l, mid, ql, qr, val);
			Modify(node * 2 + 2, mid, r, ql, qr, val);
			values[node] = calc_op(values[2 * node + 1], values[2 * node + 2]);
			apply_mod_op(values[node], operation[node], r - l);
		}
	}
	...
	int query(int node, int l, int r, int ql, int qr) {
		if (l >= qr or r <= ql) {
			...
			apply_mod_op(ans, operation[node],min(qr, r) - max(ql, l));
			return ans;
		}
	}
};
int32_t main() {
    ...
    while (q--) {
    	...
    	if (op & 1) {
    		cin >> v;
    		a.Modify(0, 0, n, l, r, v);
    	} else {
    		cout << a.query(0, 0, n, l, r) << endl;
    	}
    }
    return 0;
}
